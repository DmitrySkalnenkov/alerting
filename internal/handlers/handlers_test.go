	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			*storage.MetStorage = tt.curStorage
			...
			h := http.HandlerFunc(UpdateHandlerJson)
			h.ServeHTTP(w, req)
			res := w.Result()
...
func TestGaugeHandlerAPI1(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			h := http.HandlerFunc(UpdateGaugeHandlerPlain)
			h.ServeHTTP(w, req)
			res := w.Result()
			_, err := io.ReadAll(res.Body)
			if err != nil {
...
func TestCounterHandlerAPI1(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, tt.request, nil)
			w := httptest.NewRecorder()
			h := http.HandlerFunc(UpdateCounterHandlerPlain)
			h.ServeHTTP(w, req)
			res := w.Result()
			_, err := io.ReadAll(res.Body)
			if err != nil {
...
func TestGetGaugeHandlerAPI1(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router.HandleFunc("/value/gauge/{MetricName}", ValueGaugeHandlerGet)
...
func TestGetCounterHandlerAPI1(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router := chi.NewRouter()
			router.Use(middleware.Logger)
			router.HandleFunc("/value/counter/{MetricName}", ValueCounterHandlerGet)
			router.ServeHTTP(w, getReq)
			//GetGaugeHandler(w, getReq)
			resp := w.Result()
			bodyBytes, err := io.ReadAll(resp.Body)
